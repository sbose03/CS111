...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, double longitude, double latitude,  double[][] drywet, double[][] wetwet){  
        for (int i = 0; i < drywet.length; ++i) {    
            for (int j = 0; i < drywet.length; ++i) {
                      if (longitude  == drywet[i][0] && latitude == drywet[i][1]) {
                        drywetProbability[i] = drywet[i][j];
                      }
                  }
                }
        for (int i = 0; i < wetwet.length; ++i) {
            for(int j = 0; j< wetwet.length; ++i) {
                if (longitude== wetwet[i][0] && latitude == wetwet[i][1]) {
                    wetwetProbability[i] = wetwet[i][j];
                   }
            }
        }
    }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
            int[] monthlyForecast = new int[numberOfDays];

                int random = StdRandom.uniform(0,1);       

                if(random < .5) {
                    monthlyForecast[0] = WET;
                }
                if (random >= .5) {
                    monthlyForecast[0] = DRY;
                }


                for (int i = 0; i < monthlyForecast.length; ++i) {
                  
                      if (monthlyForecast[0] == WET) {
                        if (random <= wetwetProbability) {
                            monthlyForecast[i+1] = WET;       
                        }        
                        if (random >= wetwetProbability) {
                            monthlyForecast[i+1] = DRY;
                        }
                        
                    }

                    if (monthlyForecast[0] == DRY) {
    ...
    public static int[] oneMonthForecast (int numberOfLocations, int month, double longitude, double latitude) {
        double[][] drywets = new double[4100][14];
        double[][] wetwets = new double[4100][14];

        double[] drywetsProbability = new double[12];
        double[] wetwetsProbability = new double[12]; 

        
       
        populateArrays(drywets,wetwets);
        populateLocationProbabilities(drywetsProbability, wetwetsProbability, longitude, latitude, drywets, wetwets);
        int [] forecast = new int[12];
        forecast = forecastGenerator(drywetsProbability[month],wetwetsProbability[month],numberOfDaysInMonth[month]);
        return forecast;
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
       ...
       for (int i = 1; i < forecast.length; ++i) {
           ...
           if (spellLength < val && forecast[i-1] == mode) {
               spellLength = val;
           }
           val = 1;
       }
       if (spellLength < val && forecast[forecast.length-1] == mode) {
            spellLength = val;
       }
       return spellLength; 
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for (int i = 0; i < x; i++) {
            ...
            val = dryStreak;
            dryStreak = numberOfWetDryDays(arr, 2);

            if(dryStreak > val){
                index = i;
            } 
            else if (val >  dryStreak){
                dryStreak = val;
            } 
            else{
                dryStreak = val;
